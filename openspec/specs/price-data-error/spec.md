...
Defines `PriceDataError` — the exception raised by `Runner.__init__` when the supplied price data fails validation (missing tickers or insufficient date range).

**Module:** `moa_allocations/engine/runner.py`
**Exported from:** `moa_allocations/engine/__init__.py`
