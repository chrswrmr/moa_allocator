...
Defines the four query modes (`--validate`, `--tickers`, `--check-prices`, `--list-indicators`) as CLI entry points, their mutual exclusivity, JSON output schemas, and the public API functions that back them.

**Module:** `main.py`, `moa_allocations/__init__.py`
