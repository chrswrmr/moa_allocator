...
**Purpose:** Snap arbitrary calendar dates to the nearest in-calendar trading day using the pidb_ib date index, applied to `start_date` and `end_date` before any price fetch or engine call.
...
- **WHEN** `_snap_to_trading_day("2024-01-06", "forward", "SPY", db_path)` is called and `2024-01-06` is a Saturday
- **THEN** it SHALL return `"2024-01-08"` (the next trading day)

#### Scenario: end_date on a non-trading day snaps backward
- **WHEN** `_snap_to_trading_day("2024-12-25", "backward", "SPY", db_path)` is called and `2024-12-25` is not in the pidb_ib calendar
