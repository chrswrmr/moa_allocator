...
- [x] 2.1 Implement `_default_price_fetcher(tickers, start_date, end_date, db_path)` in `moa_allocations/__init__.py` — accepts `db_path` directly, no env var
...
- [x] 4.1 Test `run()` end-to-end with a custom `price_fetcher` (mock that returns a valid `pd.DataFrame`) — verify full pipeline executes and returns allocations
- [x] 4.2 Test lookback date calculation: `max_lookback=200` → `ceil(200*7/5)+10 = 290` calendar days subtracted
- [x] 4.3 Test lookback date calculation: `max_lookback=0` → 10 calendar days subtracted (buffer only)
