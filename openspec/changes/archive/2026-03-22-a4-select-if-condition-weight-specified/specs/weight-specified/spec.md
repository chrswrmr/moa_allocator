...
`WeightSpecified` SHALL accept a `custom_weights` dict (`{node_id: float}`) at init time. On `__call__`, it SHALL write `custom_weights` directly to `target.temp['weights']` and SHALL always return `True`. No runtime validation of weight sums is performed — the compiler guarantees weights sum to `1.0`.
