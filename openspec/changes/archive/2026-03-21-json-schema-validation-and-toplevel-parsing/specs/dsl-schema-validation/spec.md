...
After schema validation passes, `compile_strategy()` SHALL make the four top-level fields — `id`, `version-dsl`, `settings`, `root_node` — available for subsequent steps (C3 semantic validation and C4 node instantiation).

#### Scenario: All top-level fields are accessible after validation
- **WHEN** JSON loading and schema validation both succeed
- **THEN** `id`, `version-dsl`, `settings`, and `root_node` are extracted from the parsed document and available to the next step
